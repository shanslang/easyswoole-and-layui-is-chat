    ...
    <?php foreach($list as $k=>$v): ?>
        ...
        <li data-uid="<?=$this->e($v['uid'])?>" data-fromgroup="<?=$this->e($v['group_id'])?>">
            ...
            <p class="layim-msgbox-user">
                <a href="javascript:;" ><?=$this->e($v['nickname'])?></a>
                <span><?=$this->e($v['time'])?></span></p>
            <p class="layim-msgbox-content">申请加入群 <b><?=$this->e($v['groupname'])?></b>
                <span>附言: <?=$this->e($v['remark'])?></span></p>
            <p class="layim-msgbox-btn">
                <?php if ($v['status'] == 0):?>
                <button class="layui-btn layui-btn-small" onclick='joinGroup(<?=$this->e($v['id'])?>,<?=$this->e($v['gid'])?>,<?=$this->e($v['from_id'])?>,<?=$this->e($v['user_id'])?>)'>同意</button>
                <button class="layui-btn layui-btn-small layui-btn-primary" onclick="refuse(<?=$this->e($v['id'])?>,$(this))">拒绝</button>
                <?php else:?>
                <span>已<?php echo $v['status'] == 1 ? '同意' : '拒绝'?></span>
                <?php endif;?>
...
<script>
        ...
        $.ajax({
            ...
            success:function (res) {
                if (res.code == 200){
                    ...
                    parent.sendMsg(parent.socket,JSON.stringify({type:"refuseFriend",id:id}))
                }else{
    ...
    function joinGroup(id, gid, uid, fid) {
        var storage=window.localStorage;
        $.ajax({
            url:"/Chat/User/joinGroup2",
            type:"post",
            data:{groupid:gid,uid:uid,id:id, fid:fid},
            dataType:"json",
            success:function (res) {
                console.log(res)
                if(res.code == 200){
                    ...
                    var joinNotify = {type:"joinNotify","groupid":gid,token:storage.getItem('token')}
                    parent.sendMsg(parent.socket,JSON.stringify(joinNotify));
                }else{
                    layer.msg(res.msg,function () {})
                }
            },
                ...
                $.ajax({
                    ...
                    success:function (res) {
                        ...
                        if (res.code == 200){
                            ...
                            parent.sendMsg(parent.socket, JSON.stringify({type:"addList",id:uid,fromgroup:fromgroup,token:storage.getItem('token')}))//通知对方，我已同意，把我加入到对方好友列表并添加消息提醒
