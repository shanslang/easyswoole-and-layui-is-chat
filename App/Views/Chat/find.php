                ...
                <?php foreach($group_list as $k=>$v): ?>
                    ...
                    <div class="layui-col-md4" style="border-bottom: 1px solid #f6f6f6">
                        <div class="layui-card">
                            ...
                            <div class="layui-card-body">
                                <img style="width: 75px;height: 75px;object-fit: cover;" src="<?=$this->e($v['avatar'])?>" alt="">
                                <button onclick="joinGroup(<?=$this->e($v['id'])?>, '<?=$this->e($v['groupname'])?>','<?=$this->e($v['avatar'])?>')" style="float: right" class="layui-btn layui-btn-normal layui-btn-sm">
...
<script>
    ...
    function findUser() {
        wd = $('#user-wd').val();
        window.location.href="/Chat/User/find?type=user&wd="+wd
    }
    function findGroup() {
        wd = $('#group-wd').val();
        window.location.href="/Chat/User/find?type=group&wd="+wd
    }
        ...
        layui.use('layim', function(layim){
            layim.add({
                ...
                ,submit: function(group, remark, index){ //一般在此执行Ajax和WS，以通知对方
                    ...
                    parent.sendMsg(parent.socket,JSON.stringify(data))
    ...
    function joinGroup(id, groupname, avatar) {
        layui.use('layim', function(layim){
            layim.add({
                ...
                ,avatar: avatar //头像
                ,submit: function(group, remark, index){ //一般在此执行Ajax和WS，以通知对方
                    var data = {type:"addGroup",to_group_id:id,to_friend_group_id:group,remark:remark,token:storage.getItem('token')};
                    parent.sendMsg(parent.socket,JSON.stringify(data));
                    console.log(group); //获取选择的好友分组ID，若为添加群，则不返回值
                    console.log(remark); //获取附加信息
                    layer.close(index); //关闭改面板
    ...
    function createGroup() {
        layer.open({
            ...
            shade: 0.8,
            area: ['40%', '70%'],
            content: '/Chat/User/createGroup' //iframe的url
        });
    }
</script>
</body>
